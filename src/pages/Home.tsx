...
const Home: React.FC = () => {
  ...
  return (
    <div className='flex flex-col h-screen bg-black text-white justify-center items-center'>
      ...
      {isThinking && <div>Pensando...</div>}
      {response && !isThinking && <div>Resposta: {response}</div>}
      <form className='flex flex-col items-center my-8' onSubmit={handleSubmit}>
        <input placeholder='Insira aqui sua pergunta!' required className='p-2 w-60 rounded-xl text-center' type="text" name="text" id="text" value={inputValue} onChange={handleInputChange} />
        <button className='bg-green-500 hover:bg-green-700 duration-500 p-2 mt-2 rounded-xl w-24 text-center' type="submit">Perguntar</button>
      </form>
    </div>
  );
};
